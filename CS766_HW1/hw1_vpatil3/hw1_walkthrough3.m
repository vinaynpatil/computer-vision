...
% Display the image
figure, imshow(iheartny_img, 'InitialMagnification', 'fit');

% Convert the color image into a grayscale image
gray_iheartny_img = rgb2gray(iheartny_img);

% Display the image
figure, imshow(gray_iheartny_img, 'InitialMagnification', 'fit');
...
imshow(resized_mask, 'InitialMagnification', 'fit');

% Invert the mask
iresized_mask = ~resized_mask; imshow(iresized_mask, 'InitialMagnification', 'fit');
...
imshow(iresized_mask, 'InitialMagnification', 'fit'); 
...
adding_pad = zeros(size(small_nyc, 1),size(small_nyc, 2)) ;
adding_pad(1:size(iresized_mask, 1),1:size(iresized_mask, 2)) = iresized_mask;

iresized_mask = adding_pad;
...
figure, imshow(love_small_nyc, 'InitialMagnification', 'fit');
