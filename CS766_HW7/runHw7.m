...
signAcademicHonestyPolicy('Vinay Patil', 'vpatil3');
...
search_half_window_size = 40;   % Half size of the search window
template_half_window_size = 20; % Half size of the template window
grid_MN = [48, 64];              % Number of rows and cols in the grid
...
% to specify the target of interest
%tracking_params.rect = chooseTarget(data_params);
tracking_params.rect = [154 130 49 47];
...
% to specify the target of interest
%tracking_params.rect = chooseTarget(data_params);
tracking_params.rect = [316 223 33 109];
